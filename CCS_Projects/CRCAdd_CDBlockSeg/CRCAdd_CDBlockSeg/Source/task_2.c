...
int Int_matrix[3][188] = {
    ...
    {3, 7, 19, 7, 7, 11, 5, 11, 7, 41, 103, 15, 9, 17,
     9, 21, 101, 21, 57, 23, 13, 27, 11, 27, 85, 29,
     33, 15, 17, 33, 103, 19, 19, 37, 19, 21, 21,
     115, 193, 21, 133, 81, 45, 23, 243, 151,
     155, 25, 51, 47, 91, 29, 29, 247, 29, 89,
     91, 157, 55, 31, 17, 35, 227, 65, 19, 37,
     41, 39, 185, 43, 21, 155, 79, 139, 23, 217,
     25, 17, 127, 25, 239, 17, 137, 215, 29, 15,
     147, 29, 59, 65, 55, 31, 17, 171, 67, 35,
     19, 39, 19, 199, 21, 211, 21, 43, 149, 45,
     49, 71, 13, 17, 25, 183, 55, 127, 27, 29,
     29, 57, 45, 31, 59, 185, 113, 31, 17, 171,
     209, 253, 367, 265, 181, 39, 27, 127, 143,
     43, 29, 45, 157, 47, 13, 111, 443, 51, 51,
     451, 257, 57, 313, 271, 179, 331, 363,
     375, 127, 31, 33, 43, 33, 477, 35, 233, 357,
     337, 37, 71, 71, 37, 39, 127, 39, 39, 31, 113,
     41, 251, 43, 21, 43, 45, 45, 161, 89, 323, 47, 23, 47, 263},
    {10, 12, 42, 16, 18, 20, 22, 24, 26, 84, 90, 32, 34,
     108, 38, 120, 84, 44, 46, 48, 50, 52, 36, 56, 58,
     60, 62, 32, 198, 68, 210, 36, 74, 76, 78, 120, 82,
     84, 86, 44, 90, 46, 94, 48, 98, 40, 102, 52, 106,
     72, 110, 168, 114, 58, 118, 180, 122, 62, 84, 64,
     66, 68, 420, 96, 74, 76, 234, 80, 82, 252, 86, 44,
     120, 92, 94, 48, 98, 80, 102, 52, 106, 48, 110,
     112, 114, 58, 118, 60, 122, 124, 84, 64, 66, 204,
     140, 72, 74, 76, 78, 240, 82, 252, 86, 88, 60, 92,
     846, 48, 28, 80, 102, 104, 954, 96, 110, 112, 114,
     116, 354, 120, 610, 124, 420, 64, 66, 136, 420,
     216, 444, 456, 468, 80, 164, 504, 172, 88, 300,
     92, 188, 96, 28, 240, 204, 104, 212, 192, 220, 336,
     228, 232, 236, 120, 244, 248, 168, 64, 130, 264,
...
int TurboEncode(int *pinputbit, int *ptempbit, int *poutputbit, int C, int Kp, int Km, int Cm, int F)
{
    ...
    for (i = 0; i < C; i++)
    {
        ...
        for (j = 0; j < 4; j++)
        {
            ...
            poutputbit[i * (Kp + 4) * 3 + Kp + 4 + j + data_len[i]] = tail_bits[index[j + 4]];
            poutputbit[i * (Kp + 4) * 3 + 2 * (Kp + 4) + j + data_len[i]] = tail_bits[index[j + 8]];
        }
        if (i == 0 && F > 0)
        {
            for (j = 0; j < F; j++)
...
int LTE_RateMatch(int *input_codedata, int *rmdata, int *rmlen, int channel_type, int Nir, int C, int Cm,
                  int direction, int module_type, int Rvdix, int Nl, int G, int Km, int Kp)
{
    int i = 0;
    // 计算每个码块的编码长度
...
int SubblockInterleaver(int *input_bits, int *output_bits, int input_len, int intl_mode)
{
    ...
    {
        int tmp[32] = {0, 16, 8, 24, 4, 20, 12, 28,
    ...
    {
        int tmp[32] = {1, 17, 9, 25, 5, 21, 13, 29,
                       3, 19, 11, 27, 7, 23, 15, 31,
